...
P 제어 시스템의 오차는 다음과 같이 구할 수 있습니다.
...
따라서 P 제어만으로는 오차를 $$0$$으로 만들 수 없습니다.
...
I 제어 시스템의 오차는 다음과 같이 구할 수 있습니다.
...
I 제어를 통해 정상 상태 오차를 제거할 수 있음을 알 수 있습니다.
제어 사이클을 거듭하며 오차 함수가 $$0$$이 되면 더 이상 오차가 누적되지 않고, 오차가 $$0$$이 된 시점의 제어기 출력이 시스템에 계속 입력됩니다.

### 비례적분 제어(PI 제어)

P 제어는 정상 상태 오차를 $$0$$으로 만들 수 없고, I 제어는 느리다는 단점이 있습니다.
...
루프 이득의 극점은 전동기의 파라미터 $$R_a,L_a$$로 결정되고, 영점은 PI 제어기의 이득으로 결정됩니다.
PI 제어기의 이득을 다음과 같이 설정해봅시다.
...
PI 제어기의 이득을 조정하여 차단 주파수를 조정할 수 있습니다.
...
PI 제어기를 지나 피드포워드 제어(역기전력 보상)가 된 전기자 전압이 제한기를 거쳐서 입력되는 구조입니다.
...
두번째 항의 영점과 극점에 의한 동특성에 의해 과도 현상이 발생합니다.
따라서 극점-영점 상쇄를 통해 동특성을 제거할 필요가 있습니다.
이는 다음과 같이 제거할 수 있습니다.

$$
\begin{align*}
	&K_a=\frac{1}{k_p}\\
	&\rightarrow i=\frac{\omega_{cc}}{s+\omega_{cc}}\left(i^*-\frac{1}{k_p}\left(v^*-v\right)\right)
\end{align*}
$$

이 과정을 통해 제어기는 안티 와인드업을 수행합니다.

### 전류 제어기 설계

이제 전류 제어기를 어떻게 설계하는지 정리해보겠습니다.
1. 구동할 시스템의 스위칭 주파수를 결정합니다.
2. 전류 제어기의 대역폭을 결정합니다. 보통 스위칭 주파수의 $$\displaystyle\frac{1}{15}-\displaystyle\frac{1}{40}$$ 배로 설정합니다.
3. PI 이득을 차단 주파수와 전동기의 파라미터를 통해 설정합니다: $$k_{pc}=L_a\omega_{cc},k_{ic}=R_a\omega_{cc}$$.
4. 안티 와인드업의 이득을 설정합니다: $$k_a=\displaystyle\frac{1}{k_{pc}}$$

---

## 속도 제어기

전류를 제어하여 토크를 제어했으니 다음으로 속도를 제어해야 합니다.
부하에 가해지는 마찰을 부하 토크에 포함시킴으로써 속도 제어기를 다음과 같이 나타낼 수 있습니다.
(이하 관성 모멘트는 전류와의 혼동을 방지하기 위해 $$J$$로 표기합니다.)

<figure style="text-align: center;">
  <img src="./PEFigure/속도제어기.svg" alt="속도제어기" width="100%"/>
  <figcaption style="text-align: center; margin-top: 8px; font-size: 0.9em; color: #555;">
    (그림. 속도 제어기)
  </figcaption>
</figure>

속도 제어기 또한 다음과 같이 PI 제어를 이용합니다.

$$
G_s(s)=K_{ps}+\frac{K_{is}}{s}
$$

이 시스템의 루프 이득은 다음과 같습니다.

$$
G_L(s)=\left(K_{ps}+\frac{K_{is}}{s}\right)\frac{\omega_{cc}}{s+\omega_{cc}}\frac{K_T}{sJ}
$$

전류 제어기가 대역폭이 $$\omega_{cc}$$인 1차 지연 요소임을 알 수 있습니다.

### 속도 제어기 설계

루프 이득을 보드 선도에 나타내면 다음과 같습니다.

<figure style="text-align: center;">
  <img src="./PEFigure/속도제어기보드.svg" alt="속도제어기보드" width="100%"/>
  <figcaption style="text-align: center; margin-top: 8px; font-size: 0.9em; color: #555;">
    (그림. 속도 제어기 루프 이득의 보드 선도)
  </figcaption>
</figure>

속도 제어기의 대역폭을 $$\omega_{cs}$$라고 해봅시다.
전류 제어기의 응답 속도는 속도 제어기의 응답에 비해 충분히 빨라야 합니다.
속도 제어기의 제어 대역은 전류 제어기의 차단 주파수에 비해 매우 작다고 볼 수 있습니다.
따라서 전류 제어기의 전달 함수를 다음과 같이 근사할 수 있습니다.

$$
G_c(s)=\frac{\omega_{cc}}{s+\omega_{cc}}\bigg\vert_{s=j\omega_{cs}}\approx1
$$

다음으로 PI 제어기를 살펴봅시다.

$$
G_{PI}(s)=K_{ps}+\frac{K_{is}}{s}=K_{ps}\left(1+\frac{\omega_{PI}}{s}\right)
$$

여기서 $$\omega_{PI}=\displaystyle\frac{K_{is}}{K_{ps}}$$는 PI 제어기의 영점입니다.
이 영점 속도 제어기의 대역폭보다 충분히 작게 설정하면 다음과 같이 근사할 수 있습니다.

$$
G_{PI}(s)=K_{ps}\left(1+\frac{\omega_{PI}}{s}\right)\approx K_{ps}
$$

보통 영점은 차단 주파수의 $$\displaystyle\frac{1}{10}-\displaystyle\frac{1}{5}$$배로 설계합니다.
영점이 너무 크면 우선 위상 여유가 작아지고, 이득 여유 또한 작게 만들어 안정도가 나빠지고, 과도 응답에 문제가 생깁니다.
영점이 너무 작으면 적분 이득이 줄어드는 것이므로 제어 성능이 나빠져 정상 상태 오차 제거에 문제가 생깁니다.
따라서 적절한 영점 설계가 필요합니다.

위 과정을 거친 속도 제어기의 루프 이득은 다음과 같이 간단해집니다.

$$
G_L(s)=\left(K_{ps}+\frac{K_{is}}{s}\right)\frac{\omega_{cc}}{s+\omega_{cc}}\frac{K_T}{sJ}\approx K_{ps}\frac{K_T}{sJ}
$$

속도 제어기의 대역폭인 $$\omega_{cs}$$에서는 전달 함수의 크기가 $$1$$입니다.
따라서 비례 이득은 다음과 같습니다.

$$
\begin{align*}
&\left\vert G_L(j\omega_{cs})\right\vert=K_{ps}\frac{K_T}{J\omega_{cs}}=1\\
&\rightarrow K_{ps}=\frac{J\omega_{cs}}{K_T}
\end{align*}
$$

PI 제어기의 영점을 설계했고, 비례 이득을 구했으니 적분 이득은 다음과 같이 구할 수 있습니다.

$$
K_{is}=\omega_{PI}K_{ps}
$$

마지막으로 속도 제어기의 차단 주파수는 제어기의 샘플링 주파수의 $$\displaystyle\frac{1}{20}-\displaystyle\frac{1}{10}$$배로 설계하면 됩니다.
