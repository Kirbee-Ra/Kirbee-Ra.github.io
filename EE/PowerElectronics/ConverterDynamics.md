...
시간 영역 성능은 컨버터를 전압원으로 취급할 때, 중요한 지표입니다.
이는 앞서 언급한 바와 같이 입력 전압이나 부하 전류의 변동에 따라 출력 전압이 얼마나 제어되는지 보는 것입니다.
...
부하는 그대로지만 전달되는 전력이 줄었으므로 출력 전압이 줄어듭니다.
...
이런 경우는 교차 주파수가 곧 특성 방정식의 해가 됩니다.
복소수는 항상 켤레 근이 있으므로 특성 방정식은 $$s=\pm j\omega_c$$의 근을 갖습니다.
이 식을 정리하면 교차 주파수의 주파수로 진동하는 형태가 됩니다.
감쇠 항이 없어서 진폭이 일정하므로 제어할 수 없습니다.
...
동작점이 변할 때, 루프 이득의 크기가 변하는 경우가 있습니다.
일반적인 증폭기의 경우, 포화 지점 바깥 영역에서 동작을 하게 되면 증폭 비가 줄어듭니다.
...
그래서 루프 이득의 이득 여유와 위상 여유를 설정하는 기준이 있습니다.

1. 이득 여유는 $$6\ \text{dB}$$를 넘도록 설계하는 것이 좋습니다.
2. 위상 여유는 $$60^{\circ}$$ 정도가 가장 좋지만, 보통 $$45^{\circ}$$만 넘도록 설계합니다. 그리고 $$30^{\circ}$$ 아래로는 떨어지지 않는 것이 좋습니다.

여유를 확보한다는 것은 외란에 대한 대비라고 볼 수도 있지만, 동작의 불확실성에 대한 대비이기도 합니다.
여유를 설계할 때 이용된 보드 선도는 실제 시스템에 대한 근사입니다.
오차가 존재하므로 여유를 많이 확보할 수록 오차에 대한 대비 또한 확실하게 할 수 있습니다.

### 이득의 상충 관계

여유를 그저 많이 확보한다고 해서 시스템이 더 좋아지는 것은 아닙니다.
루프 이득의 이득 여유를 크게 설계한다는 것은 교차 주파수를 줄인다는 의미입니다.
이는 곧 [전압 모드 제어](./ConverterVMC)에서의 제어기 설계에 영향을 미칩니다.
음파 민감도와 출력 임피던스의 크기를 모든 주파수 대역에서 줄여야 합니다.
교차 주파수가 낮다면, 감쇠가 충분히 되지 않습니다.
또한 감쇠할 수 있는 대역 또한 줄어듭니다.
그리고 정착 시간 또한 길어집니다.
위상 여유 또한 마찬가지입니다.
이렇게 여유와 동 특성은 서로 상충 관계에 있습니다.
