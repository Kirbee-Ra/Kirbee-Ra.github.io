...
$$
\Lambda_a(t)=\Lambda_0\cos\theta_a
$$

자석을 일정한 각속도 $$\omega$$로 회전시키면 다음과 같이 나타낼 수 있습니다.

$$
\Lambda_a(t)=\Lambda_0\cos\left(\omega t\right)
$$
...
$$
	\begin{align*}
v_a(t)&=-\frac{d\Lambda_a}{dt}\\
&=\omega\Lambda_0\sin\left(\omega t\right)\\
&=V_{ph}\sin\left(\omega t\right)
\end{align*}
$$

이 유도 기전력이 바로 $$a$$상의 상전압입니다.
$$b$$상과 $$c$$상은 $$a$$상과 위상이 다르므로 그림과 같이 $$120^{\circ}$$씩 간격을 두어 배치합니다.

(3 ph gen)

이와 같이 물리적으로 $$120^{\circ}$$씩 간격을 두어 배치를 하면, 쇄교 자속이 각 권선을 통과하는 각이 달라지므로 3상 교류 발전기를 구현할 수 있습니다.
