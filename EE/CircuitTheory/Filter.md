...
RLC 회로의 경우, 임피던스의 크기가 최소(직렬 공진 회로) 또는 최대(병렬 공진 회로)가 되는 지점에서 공진이 일어납니다.
즉, 전압이 최대 또는 최소가 되는 지점이므로 이 또한 차단 주파수를 이용하여 계산할 수 있습니다.
대역 필터나 대역 차단 필터의 전달 함수의 크기는 보드 선도 상에서 공진 주파수를 기준으로 좌우 대칭적입니다.
따라서 로그 스케일의 두 차단 주파수의 산술 평균으로 구해보면 다음과 같습니다.


$$
\begin{align*}
		&\frac{20\log\omega_{c1}+20\log\omega_{c2}}{2}=20\log\omega_0\\
		&\rightarrow\omega_0=\sqrt{\omega_{c1}\omega_{c2}}
	\end{align*}
$$

이와 같이 공진 주파수는 두 차단 주파수 사이의 기하 평균으로 구할 수 있습니다.

### 대역폭

우리가 얻고자 하거나(대역 필터) 차단하고자 하는(대역 차단 필터) 주파수 영역이 있고, 그 영역의 간격인 **대역폭(Bandwidth)**을 차단 주파수를 통해 구할 수 있습니다.
말 그대로 두 주파수의 차이를 구하면 됩니다.
...
**Q 인자(Q-Factor, Quality Factor)**는 필터의 성능을 평가할 수 있는 지표 중 하나입니다.
다음과 같이 정의됩니다.
...
위 식을 보면 알 수 있듯이 Q 인자가 크면 공진 주파수를 중심으로 통과 대역이 좁은 것을 알 수 있습니다.
이렇게 되면 우리가 원치 않는 주파수를 더 잘 거를 수 있습니다.
하지만 응답 속도가 느려진다는 단점도 존재합니다.
...
다음으로 이와 같은 특정 주파수를 지닌 사인파가 입력되었다고 해봅시다.
...
$$
\begin{align*}
\mathbf{Y}&=H(j\omega)\mathbf{X}\\
&=\left\vert H(j\omega)\right\vert X_m\angle\left(H(j\omega)+\theta_x\right)
\end{align*}
$$

크기는 필터에 의해 스케일링 되고, 위상은 변하는 것을 알 수 있습니다.
스케일링을 통해 원하는 주파수 대역만 큰 신호로 통과하게 설계할 수 있습니다.
...
- [수동 필터](./PassiveFilter.md)
- [능동 필터]
- [보드 선도](../ControlEngineering/BodePlot.md)
